...
const cvFormatter = (arr) => {
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        if (eachArray.yearsOfExperience > 1) {
            let fullName = "";
            if (eachArray.firstName !== null && eachArray.lastName !== null) {
                fullName += `${eachArray.firstName} ${eachArray.lastName}`;
            }
            else if (eachArray.firstName !== null) {
                fullName += eachArray.firstName;
            }

            else if (eachArray.lastName !== null) {
                fullName += eachArray.lastName;
            }
...
const applicationsStatics = (arr) => {
    ...
    for (i = 0; i < arr.length; i++) {
        ...
        if ((firstName && lastName) === null || yearsOfExperience <= 1) {
            eachArray.rejectedApplicants += 1;
        }
        else {
            eachArray.totalApplicants += 1;

            switch (tech) {
                case 'python':
                    eachArray.python_devs+=1;
                    break;

                case 'JS':
                    eachArray.javaScript_devs+=1;

                    break;

                case '.Net':
                    eachArray.dotNet_devs+=1;

                    break;


                case 'Java':
                    eachArray.java_devs+=1;
...
// function Objfilter(python,javaScript,dotNet,java,total,rejected)
// {
//     this.python_devs = python  ;
//     this.javaScript_devs= javaScript ;
//     this.dotNet_devs= dotNet ;
//     this.java_devs= java ;
//     this.totalApplicants= total ;
//     this.this.rejectedApplicants= rejected ;
// }
...
const classesAvg = (data) => {
    ...
    for(let i=0 ; i<data.grades.length;i++)
    {
        ...
        {
            let classIn = grade.classes[j];
            let sum = 0; 

            for(let n = 0 ; i<classIn.classScores.length ; n++)
            {
                sum = sum + classIn.classScores[n];
            }

            let avgGrade = sum / classIn.classScores.length;
            avgGrade = avgGrade - (avgGrade%1)

            classIn.avg = avgGrade;
        }
    }
